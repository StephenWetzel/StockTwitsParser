...
for symbol in symbols:
	futureType = "Crude Oil"
	expireMonth = 1
	expireYear = 2016
	stillUpdated = 1
	#first we have to make sure the symbol is in the parent table (Symbols) or inserts into the child (Prices) will fail
	sql = "INSERT INTO Symbols(exchange, symbol, type, expire_month, expire_year, updated) VALUES ('%s', '%s', '%s', '%s', '%s', '%s') ON DUPLICATE KEY UPDATE exchange=VALUES(exchange), type=VALUES(type), expire_month=VALUES(expire_month), expire_year=VALUES(expire_year), updated=VALUES(updated)" % \
	(exchange, symbol, futureType, expireMonth, expireYear, stillUpdated)
	cur.execute(sql)
